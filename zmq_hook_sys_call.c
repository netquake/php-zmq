...
static socket_pfn_t g_sys_socket_func   = 0;
static connect_pfn_t g_sys_connect_func = 0;
static close_pfn_t g_sys_close_func     = 0;
static read_pfn_t g_sys_read_func       = 0;
static write_pfn_t g_sys_write_func     = 0;
static sendto_pfn_t g_sys_sendto_func   = 0;
static recvfrom_pfn_t g_sys_recvfrom_func = 0;
static send_pfn_t g_sys_send_func       = 0;
static recv_pfn_t g_sys_recv_func       = 0;
static poll_pfn_t g_sys_poll_func       = 0;
static setsockopt_pfn_t g_sys_setsockopt_func = 0;
static fcntl_pfn_t g_sys_fcntl_func     = 0;
static setenv_pfn_t g_sys_setenv_func   = 0;
static unsetenv_pfn_t g_sys_unsetenv_func = 0;
static getenv_pfn_t g_sys_getenv_func   = 0;
static __res_state_pfn_t g_sys___res_state_func  = 0;
static gethostbyname_pfn_t g_sys_gethostbyname_func = 0;
static __poll_pfn_t g_sys___poll_func = 0;
...
static inline struct rpchook_t * alloc_by_fd( int fd )
{
	if( fd > -1 && fd < (int)sizeof(g_rpchook_socket_fd) / (int)sizeof(g_rpchook_socket_fd[0]) )
	{
		struct rpchook_t *lp = (struct rpchook_t*)calloc( 1,sizeof(struct rpchook_t) );
...
/*
int query_table_(const int* table, const unsigned int size, const int key) {
...
bool insert_table_(const int* table, unsigned int& size, const int key, const int val) {
	if (size >= 63) {
		return false;
	}
...
////////////////////////////////////////////////////////////
	...
	if (nfds > 1) {
		...
		for (size_t i = 0; i < nfds; i++) {
			...
			val = query_table_( fd_table, fd_count, fds[i].fd );
			if( -1 == val ) {
				fds_merge[nfds_merge] = fds[i];
				///< m[fds[i].fd] = nfds_merge;
				insert_table_( fd_table, fd_count, fds[i].fd, nfds_merge );
				nfds_merge++;
			} else {
				///< int j = it->second;
				int j = val;
	...
	if (nfds_merge == nfds || nfds == 1) {
		ret = g_sys_poll_func(fds, nfds, timeout);
...
{
	struct stCoSysEnvArr_t *lp = (struct stCoSysEnvArr_t*)calloc( sizeof(struct stCoSysEnvArr_t),1 );	
	if( arr->cnt )
	{
